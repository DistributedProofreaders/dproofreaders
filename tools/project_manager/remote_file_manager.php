...
} elseif ($access_mode == 'self') {
    $home_dirname = "$users_rel_dir/$despecialed_username";
} elseif ($access_mode == 'disabled') {
    $home_dirname = null;
} elseif (user_is_PM() || user_is_proj_facilitator() || user_is_a_sitemanager()) {
    $home_dirname = "$users_rel_dir/$despecialed_username";
} else {
    $home_dirname = null;
}
...
function do_showdir($action_message)
{
    ...
    echo "<p>" . _("The ~/dpscans directory is not backed up so please keep a local copy on your computer until they are no longer needed. And please remove files when they're no longer required.") . "</p>";
...
function show_content()
{
    ...
    if (is_subdir_of_home($curr_relpath) || ($curr_relpath != '' && user_may_access_all_upload_dirs())) {
        ...
        if ($parent_relpath == '.' || $parent_relpath == '/') {
            $parent_relpath = '';
        }
    ...
    foreach ($item_names as $item_name) {
        ...
        if (is_file($item_path)) {
            $actions_blurb =
                get_actions_block($item_name, ['showdelete', 'download', 'showrename', 'showmove']);
            ...
            $actions_blurb =
                get_actions_block($item_name, ['showdelete', 'showrename']);
...
function get_actions_block($item_name, $valid_actions)
{
    ...
    $actions_info = [
        'showdelete' =>
            [_('Delete this item'),   'wfb_images/wfb_delete.png',   '[X]'],
        'download' =>
            [_('Download this item'), 'wfb_images/wfb_download.png', '[D]'],
        'showrename' =>
            [_('Rename this item'),   'wfb_images/wfb_rename.png',   '[R]'],
        'showmove' =>
            [_('Move this item'),     'wfb_images/wfb_move.png',     '[M]'],
    ];
