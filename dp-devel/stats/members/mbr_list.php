...
echo "<tr bgcolor='".$theme['color_headerbar_bg']."'><td colspan='6' align='center'><b><font color='".$theme['color_headerbar_font']."'>Distributed Proofreader Members</font></b></td></tr>";
echo "<tr bgcolor='".$theme['color_navbar_bg']."'>";
	if ($order == "u_id" && $direction == "asc") { $newdirection = "desc"; } else { $newdirection = "asc"; }
		echo "<td width='5%' align='center'><b><a href='mbr_list.php?".$uname."mstart=$mstart&order=u_id&direction=$newdirection'>ID</a></b></td>";
	if ($order == "username" && $direction == "asc") { $newdirection = "desc"; } else { $newdirection = "asc"; }
		echo "<td width='23%' align='center'><b><a href='mbr_list.php?".$uname."mstart=$mstart&order=username&direction=$newdirection'>Username</a></b></td>";
	if ($order == "date_created" && $direction == "asc") { $newdirection = "desc"; } else { $newdirection = "asc"; }
		echo "<td width='23%' align='center'><b><a href='mbr_list.php?".$uname."mstart=$mstart&order=date_created&direction=$newdirection'>Date Joined DP</a></b></td>";
	if ($order == "pagescompleted" && $direction == "asc") { $newdirection = "desc"; } else { $newdirection = "asc"; }
		echo "<td width='25%' align='center'><b><a href='mbr_list.php?".$uname."mstart=$mstart&order=pagescompleted&direction=$newdirection'>Total Pages Completed</a></b></td>";
	echo "<td width='23%' align='center'><b>Options</b></td>";
echo "</tr>";
if (!empty($mRows)) {
	$i = 0;
	while ($row = mysql_fetch_assoc($mResult)) {
		...
		echo "<td width='23%' align='center'><b><a href='mdetail.php?id=".$row['u_id']."'>Statistics</a>&nbsp;|&nbsp;<a href='$forums_url/privmsg.php?mode=post&u=".mysql_result($phpbbID, 0, "user_id")."'>PM</a></td>";
		echo "</tr>";
		$i++;
	}
} else {
	echo "<tr bgcolor='".$theme['color_mainbody_bg']."'><td colspan='6' align='center'><b>No more members available.</b></td></tr>";
}

echo "<tr bgcolor='".$theme['color_mainbody_bg']."'><td colspan='3' align='left'>";
if (!empty($mstart)) {
	echo "<b><a href='mbr_list.php?".$uname."order=$order&direction=$direction&mstart=".($mstart-20)."'>Previous</a></b>";
}
echo "&nbsp;</td><td colspan='3' align='right'>&nbsp;";
if ($mRows == 20) {
	echo "<b><a href='mbr_list.php?".$uname."order=$order&direction=$direction&mstart=".($mstart+20)."'>Next</a></b>";
}
echo "</td></tr>";
echo "<tr bgcolor='".$theme['color_headerbar_bg']."'><td colspan='6' align='center'><b><a href='$code_url/accounts/addproofer.php'><font color='".$theme['color_headerbar_font']."'>Create a New Account</font></a></b></td></tr>";
echo "</table><p>";
theme("", "footer");
?>