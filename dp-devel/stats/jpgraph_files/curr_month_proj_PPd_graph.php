...
if ($today['mday'] == 1 && ($today['hours'] >=0 && $today ['hours'] <= 3)) {
	if (isset($_GET['ignore_archive_graph']) && $_GET['ignore_archive_graph'] == 1) {
		...
		if (!file_exists($dynstats_dir."/graph_archive/curr_month_proj_PPd/".date("Fy",time()-86400).".png")) {
...
// Display the graph
if (isset($_GET['ignore_archive_graph']) && $_GET['ignore_archive_graph'] == 1) {
	$archiveGraphPath = $dynstats_dir."/graph_archive/curr_month_proj_PPd/".date("Fy",time()-86400).".png";
