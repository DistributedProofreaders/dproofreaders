...
if ($today['mday'] == 1 && ($today['hours'] >=0 && $today ['hours'] <= 3)) {
	if (isset($_GET['ignore_archive_graph']) && $_GET['ignore_archive_graph'] == 1) {
		...
		if (!file_exists($code_dir."/stats/graph_archive/cumulative_month_proj_created/".date("Fy",time()-86400).".png")) {
			header("Location: ".$code_url."/stats/jpgraph_files/cumulative_month_proj_created.php?ignore_archive_graph=1");
		} else {
...
// Display the graph
if (isset($_GET['ignore_archive_graph']) && $_GET['ignore_archive_graph'] == 1) {
	$archiveGraphPath = $code_dir."/stats/graph_archive/cumulative_month_proj_created/".date("Fy",time()-86400).".png";
