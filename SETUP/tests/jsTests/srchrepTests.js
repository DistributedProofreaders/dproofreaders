...
QUnit.module("srchrep tests", (hooks) => {
    ...
    QUnit.test("replaces all instances", function (assert) {
        ...
        $("#replace").val("value.");
        srchrep.doReplace();
        assert.strictEqual(window.opener.parent.docRef.editform.text_data.value, "value.", "Search replace replaces all instances.");
    ...
    QUnit.test("replaces -- instances", function (assert) {
        ...
        $("#replace").val("-");
        srchrep.doReplace();
        assert.strictEqual(window.opener.parent.docRef.editform.text_data.value, "brave - new.", "Search replace replaces all instances.");
        assert.notOk($("#undo").prop("disabled"), "Undo should be enabled.");
    });
    ...
    QUnit.test("replaces with regular expression groups correctly", function (assert) {
        ...
        srchrep.doReplace();
        assert.strictEqual(window.opener.parent.docRef.editform.text_data.value, "new __ brave.", "Search replace replaces all instances.");
        assert.notOk($("#undo").prop("disabled"), "Undo should be enabled.");
    });
    ...
    QUnit.test("restore saved text undoes replace", function (assert) {
        ...
        $("#replace").val("sch");
        srchrep.doReplace();
        assert.strictEqual(window.opener.parent.docRef.editform.text_data.value, "Example sch text.", "Search replace replaces all instances.");
        assert.notOk($("#undo").prop("disabled"), "Undo should be enabled.");
        srchrep.restoreSavedText();
        assert.strictEqual(window.opener.parent.docRef.editform.text_data.value, "Example search text.", "undo restores original text.");
