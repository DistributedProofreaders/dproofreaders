...
CREATE TABLE `biographies` (
  ...
  `last_modified` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY  (`bio_id`)
) ENGINE=MyISAM COMMENT='Contains biographies (see authors)';
# --------------------------------------------------------
...
) ENGINE=MyISAM COMMENT='Each row represents a not-yet-activated user, user_password ';
# --------------------------------------------------------
...
) ENGINE=MyISAM COMMENT='Each row represents a different PG etext';
# --------------------------------------------------------
...
CREATE TABLE `smoothread` (
  ...
  KEY `user` (`user`)
) ENGINE=MyISAM COMMENT='Each row represents an association between a user and a proj';
# --------------------------------------------------------
...
) ENGINE=MyISAM COMMENT='definitions of SPECIAL days';
# --------------------------------------------------------
